...
class ChatAgent(BaseAgent):
    ...
    Args:
        ...
        model (ModelType, optional): The LLM model to use for generating
            responses. (default :obj:`ModelType.GEMINI_1_5_FLASH`)
    ...
    def __init__(
        ...
        self.model: ModelType = (model if model is not None else ModelType.GEMINI_1_5_FLASH)
    ...
    def step(
        ...
        if num_tokens < self.model_token_limit:
            ...
            # Handle Google AI response (converted to OpenAI format)
            if not isinstance(response, dict):
                raise RuntimeError("Google AI returned unexpected struct")
