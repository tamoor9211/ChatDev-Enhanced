...
def prompt_cost(model_type: str, num_prompt_tokens: float, num_completion_tokens: float):
    ...
    input_cost_map = {
        ...
        "gemini-pro-vision": 0.00025,  # $0.25 per 1M tokens
        # Keep backward compatibility with old model names
        "gpt-3.5-turbo": 0.000075,
        "gpt-3.5-turbo-16k": 0.000075,
        "gpt-4": 0.00125,
        "gpt-4-turbo": 0.00125,
        "gpt-4o": 0.00125,
    ...
    output_cost_map = {
        "gemini-1.5-flash": 0.0003,    # $0.30 per 1M tokens
        "gemini-1.5-pro": 0.005,       # $5.00 per 1M tokens
        "gemini-1.0-pro": 0.0015,      # $1.50 per 1M tokens
        "gemini-pro": 0.0015,          # $1.50 per 1M tokens
        "gemini-pro-vision": 0.00125,  # $1.25 per 1M tokens
        # Keep backward compatibility with old model names
        "gpt-3.5-turbo": 0.0003,
        "gpt-3.5-turbo-16k": 0.0003,
        "gpt-4": 0.005,
        "gpt-4-turbo": 0.005,
        "gpt-4o": 0.005,
        "gpt-4o-mini": 0.0003,
    ...
    if model_type not in input_cost_map or model_type not in output_cost_map:
        return 0.0  # Return 0 for unknown models instead of -1
...
def get_info(dir, log_filepath):
    ...
    if os.path.exists(dir):
        ...
        if len(sublines) > 0:
            ...
            if model_type == "GEMINI_1_5_FLASH":
                model_type = "gemini-1.5-flash"
            elif model_type == "GEMINI_1_5_PRO":
                model_type = "gemini-1.5-pro"
            elif model_type == "GEMINI_1_0_PRO":
                model_type = "gemini-1.0-pro"
            elif model_type == "GEMINI_PRO":
                model_type = "gemini-pro"
            elif model_type == "GEMINI_PRO_VISION":
                model_type = "gemini-pro-vision"
            elif model_type == "GEMINI_2_0_FLASH_EXP":
                model_type = "gemini-2.0-flash-exp"
            # Keep backward compatibility for old model names
            elif model_type == "GPT_3_5_TURBO" or model_type == "GPT_3_5_TURBO_NEW":
                model_type = "gemini-1.5-flash"
