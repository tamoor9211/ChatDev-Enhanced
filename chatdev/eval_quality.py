...
def get_consistency(directory):
    ...
    def get_text_embedding(text: str):
        """Get text embedding using hybrid approach (Google AI + local fallback)."""
        try:
            ...
            embedding = hybrid_embedding.get_text_embedding(text)
            return embedding

        except Exception as e:
            print(f"Error getting text embedding: {str(e)}")
            # Use lightweight local embedding as fallback
            try:
                from local_embedding import LocalEmbedding
                local_embedding = LocalEmbedding()
                return local_embedding.get_text_embedding(text)
            except:
    ...
    def get_code_embedding(code: str):
        """Get code embedding using hybrid approach (Google AI + local fallback)."""
        try:
            # Import the hybrid embedding system
            import sys
            import os
            ...
            # Use hybrid embedding system
            hybrid_embedding = HybridEmbedding()
            embedding = hybrid_embedding.get_code_embedding(code)
            return embedding

        except Exception as e:
            print(f"Error getting code embedding: {str(e)}")
            # Use lightweight local embedding as fallback
            try:
                from local_embedding import LocalEmbedding
                local_embedding = LocalEmbedding()
                return local_embedding.get_code_embedding(code)
            except:
                # Last resort: random embedding
                import numpy as np
                np.random.seed(42)
