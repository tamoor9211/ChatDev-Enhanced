...
# Google AI Studio is now used instead of OpenAI
    ...
    def generate_images_from_codes(self):
        ...
        for filename in self.incorporated_images.keys():
            if not os.path.exists(os.path.join(self.env_dict['directory'], filename)):
                ...
                # Image generation is disabled - Google AI Studio doesn't have image generation API
                # You can integrate with other image generation services if needed
                print(f"Image generation requested for: {desc}")
                print("Note: Image generation is not available with Google AI Studio API")
